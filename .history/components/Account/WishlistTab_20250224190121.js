...
const RemoveButton = styled.button`
  ...
  &:hover {
    ...
    position: absolute;
    right: 15px; /* Pomeri srce malo više unutra */
    top: 50%;
...
const WishlistItem = styled.div`
  display: grid;
  grid-template-columns: 60px auto 30px; /* Postavi grid sa tri kolone */
  align-items: center;
  gap: 10px;
  padding: 12px;
  background: #fff;
  ...
  @media (max-width: 768px) {
    grid-template-columns: 50px auto 25px; /* Smanji dimenzije za mobilne uređaje */
    min-height: 70px;
    padding: 10px;
  }
`;
